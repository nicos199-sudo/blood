...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_orders'] = Order.objects.filter(start_date__date=timezone.now()).count()
        return context
...
def item_chart(request):
    ...
    queryset = OrderItem.objects.values('item__title').annotate(item_quantity=Sum('quantity')).order_by('-item_quantity')
    for entry in queryset:
        labels.append(entry['item__title'])
        data.append(entry['item_quantity'])
...
def pie_chart(request):
    ...
    for OrderItem in queryset:
        labels.append(OrderItem.item.title)
        data.append(OrderItem.quantity)
